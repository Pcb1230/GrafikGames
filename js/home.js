        ...
        if(this.readyState == 4 && this.status == 200){//We check that all is correct
          ...
          for(let objeto of datos){//We create a variable objeto where we put the data of the variable datos
            ...
            if(juego.length==0){
              ...
              error.appendChild(document.createTextNode("No se ha encontrado el juego introducido"));
...
function vacio(){
  let dato = document.getElementById("nombreSpecs").value;
  if(dato==""){
  let ul = document.getElementById("lista");
  ul.innerHTML="";
  }
...
function escribirforo(Usuario){
  ...
  p.appendChild(document.createTextNode(Usuario+": "+mensaje.value));
  ul.appendChild(p);
  ...
  xhttp.onreadystatechange = function(){
      if(this.readyState == 4 && this.status == 200){
        ...
        for(let objeto of datos){
          let p = document.createElement("p");
          p.appendChild(document.createTextNode("Usuario: "+objeto.mensaje));
          ul.appendChild(p);
        }
      }
    }
